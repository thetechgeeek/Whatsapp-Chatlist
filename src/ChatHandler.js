...
class ChatHandler {
  //receiving the structure for a chat, and the list of chats
  ...
  createNode(id) {
    // NODE: | prev | template/struct of data | next |
    // Creating node element
    let node = {};
    // Pointers to prev and next
    node['next'] = null;
    node['prev'] = null;
    // Create a copy of chat template. 'chat_item' is aa html item
    ...
    console.log(
      '../public/images/avatar' + eval(1 + (id % chat_img_length)) + '.png'
    );
    chat_item.querySelector('#Image').src =
      '../public/images/avatar' + eval(1 + (id % chat_img_length)) + '.png';
    //copying 'chat_item' to the node
